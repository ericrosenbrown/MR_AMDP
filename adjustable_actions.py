...
class FourRoomL1MDP(MDP):
    def __init__(self, starting_room=1, goal_room=4, starting_items=[], goal_items=[] ,gamma=0.99,actions=None):
    ...
    def _transition_func(self, state, action):
        ...
        if current_room == 1:
            if action == 'toRoom2':
                next_state = FourRoomL1State(2,items=state.items)
            if action == 'toRoom3':
                next_state = FourRoomL1State(3,items=state.items)
        if current_room == 2:
            if action == 'toRoom1':
                next_state = FourRoomL1State(1,items=state.items)
            #if action == 'toRoom4':
            #    next_state = FourRoomL1State(4,items=state.items)
            if action == 'turnOnL1':
        ...
        if current_room == 4:
            ...
            if action == 'toRoom3':
                next_state = FourRoomL1State(3,items=state.items)
            if action == 'toRoom5':
                next_state = FourRoomL1State(5,items=state.items)
        if current_room == 5:
            if action == 'toRoom4':
                next_state = FourRoomL1State(4,items=state.items)
...
def get_l1_policy(start_room=None, goal_room=None, mdp=None, starting_items=None, goal_items=None,actions=None):
    if mdp is None:
        mdp = FourRoomL1MDP(start_room, goal_room,starting_items=starting_items,goal_items=goal_items,actions=actions) #room 2, light off =0, light on =1
...
if __name__ == '__main__':
    ...
    policy = get_l1_policy(1, 5, starting_items=[0,0], goal_items=[1,1],actions=ACTIONS)
