...
class FourRoomL1MDP(MDP):
    ACTIONS = ['toRoom1', 'toRoom2', 'toRoom3', 'toRoom4', 'toRoom5','turnOnL1']
    ...
    def _transition_func(self, state, action):
        ...
        if current_room == 2:
            ...
            if action == 'turnOnL1':
                next_state = FourRoomL1State(current_room,items=[current_room,1])
        if current_room == 3:
            if action == 'toRoom4':
                next_state = FourRoomL1State(4,items=state.items)
            if action == 'toRoom1':
...
def get_l1_policy(start_room=None, goal_room=None, mdp=None):
    if mdp is None:
        mdp = FourRoomL1MDP(start_room, goal_room,starting_items=[2,0],goal_items=[2,1]) #room 2, light off =0, light on =1
